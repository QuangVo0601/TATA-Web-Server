...
Zero <-MyData[,grep("G10", invert = TRUE, colnames(MyData))]
Ones <-Zero[,grep("G11", invert = TRUE, colnames(Zero))]

One <- Ones[,grep("G1",fixed = T, colnames(Ones))] 
...
Ten <- MyData[,grepl("G10", fixed = T, colnames(MyData))]
Eleven <- MyData[,grepl("G11", fixed = "T", colnames(MyData))]
MyData <- cbind(One, Two, Three, Four, Five, Six, Seven, Eight, Nine, Ten, Eleven)

Group1 <- grep("G1",names(Ones), value= T)
...
Group10 <- grep("G10",names(MyData), value= T)
Group10 <- as.matrix(Group10)
Group10 <-gsub("G10", "Group10", Group10)
Group11 <- grep("G11",names(MyData), value= T)
Group11 <- as.matrix(Group11)
Group11 <-gsub("G11", "Group11", Group11)

condition <- rbind(Group1, Group2, Group3, Group4, Group5, Group6, Group7, Group8, Group9, Group10, Group11)
