...
MyData2 <- cbind(One, Three)
MyData3 <- cbind (One, Two, Three)
...
Group3 <- grep("G3",names(MyData3), value= T)
Group3 <- as.matrix(Group3)
Group3 <-gsub("G3", "Group3", Group3)
condition <- rbind(Group1, Group2, Group3)
condition <- sub("\\..*$", "", condition)
myfactor <- data.frame(condition)
rawdata <- readData(data=MyData3, factors=myfactor)
