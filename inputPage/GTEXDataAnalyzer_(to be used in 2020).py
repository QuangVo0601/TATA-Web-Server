...
class GTEXDataAnalyzer:
	...
	def __filter_alpha_and_fold_change(self, alpha, fc_value, test_results):
		...
		for df in test_results:
			corrected_alpha = alpha / len(df.index.values)
			temp_df = df[(df[df.columns[1]].abs() > fc_value) & (df[df.columns[3]] < corrected_alpha)]
	...
	def do_network_analysis(self, list_of_dw_vals, t_test_res):
		print("\n\n\n DOING NETWORK ANALYSIS \n\n\n")

		group_graphs = self.__convert_network_graph(list_of_dw_vals)

		graph_fig_list = []
		for network in group_graphs:
			graph_fig_list.append(self.__network_graph(network))
			print(nx.density(network))
			print("\n\n")

		for idx, fig in enumerate(graph_fig_list,1):
			fig.write_image(self.b_str_path + "/" + self.job_code_mgr.get_job_code() + "/results/" + self.group_names[0] + " vs. " + self.group_names[idx] + " graph.png")	

		network_info_list = self.__make_network_output(group_graphs, t_test_res, list_of_dw_vals)

		for idx, net_info in enumerate(network_info_list, 1):
			net_info[1].to_csv(self.b_str_path + "/" + self.job_code_mgr.get_job_code() + "/results/" + self.group_names[0] + " vs. " + self.group_names[idx] + " network edgelist.csv")
			net_info[2].to_csv(self.b_str_path + "/" + self.job_code_mgr.get_job_code() + "/results/" + self.group_names[0] + " vs. " + self.group_names[idx] + " network info.csv")

		return network_info_list

	def __make_network_output(self, group_graphs, t_test_res, dw_dfs):
		network_output_list = []
		for g, t, dw, in zip(group_graphs, t_test_res, dw_dfs):
			network_output_list.append(self.__make_network_dfs(g, t, dw))

		# print(network_output_list)

		return network_output_list	

	def __make_network_dfs(self, graph, t_res, dw_df):
		network_df_list = []

		cols = dw_df.columns
		sig_con = dw_df.apply(lambda x: x != 0)
		sig_con = sig_con.apply(lambda x: list(cols[x.values]), axis=1)
		# print(sig_con)

		node_df = self.__make_dw_edgelist(dw_df)
		# print(node_df)

		graph_density = nx.density(graph)
		graph_clus_coeff = nx.average_clustering(graph)
		node_num = nx.number_of_nodes(graph)
		edge_num = nx.number_of_edges(graph)
		net_info = {"graph density": [graph_density], "average clustering coefficient": [graph_clus_coeff], "number of nodes" : [node_num], "number of edges": [edge_num]}
		network_info_df = pd.DataFrame(net_info) 

		return [sig_con, node_df, network_info_df]

	def __make_dw_edgelist(self, dw_df):
		node_connections = [(tf_ens, sig_ens, dw_df[sig_ens].loc[tf_ens]) for tf_ens in dw_df.index.values for sig_ens in dw_df.columns.values if dw_df[sig_ens].loc[tf_ens] != 0]
		return pd.DataFrame(node_connections, columns = ["tf ens", "sig ens", "r weight"])

	def __convert_network_graph(self, sig_r_dfs):
		group_graphs = []
		for df in sig_r_dfs:
			node_df = self.__make_dw_edgelist(df)
			dw_graph = nx.from_pandas_edgelist(node_df, "tf ens", "sig ens", "r weight")
			group_graphs.append(dw_graph)
		return group_graphs

	def __network_graph(self, network_of_tf_and_sig):
		G = network_of_tf_and_sig
		# G = nx.from_pandas_dataframe(delta_frame)
		...
		for node, adjacencies in enumerate(G.adjacency()):
			...
			node_text.append('Name: ' + str(adjacencies[0]))
			node_text.append('# of connections: '+str(len(adjacencies[1])))
			####node_text.append('ENSG ID: '+str('Name:'(adjacencies[1])))
	...
	class JobCodeManager:
		def __init__(self, jcode_length, storage_path):
			...
			#looping until a valid jobcode is created
			while True:	
				try:
		...
		#updates progress.csv based on the stage num passed through
		#the hard-coded 4 should be changed to some variable when the number of stages are more well defined in the project
